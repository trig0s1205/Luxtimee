...
export class WholesaleAccessGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const session =
      await this.wholesaleAccessService.requireAccessFromToken(token);
    (req as Request & { wholesaleSession?: typeof session }).wholesaleSession =
      session;
    return true;
  }
}
...
export function getWholesaleSession(req: Request) {
  return (req as Request & { wholesaleSession?: { id: string } })
    .wholesaleSession;
}
