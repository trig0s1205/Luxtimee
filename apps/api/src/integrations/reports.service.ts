...
export class ReportsService {
  ...
  private drawTotalRow(
    ...
    doc
      .rect(PG_MARGIN, y, CONTENT_W, ROW_H)
      .fillAndStroke(BRAND.goldLight, BRAND.gold);
    for (const cell of cells) {
      doc
        .font('Helvetica-Bold')
        .fontSize(7.5)
  ...
  private addFooters(doc: PDFKit.PDFDocument, byLine: string): void {
    ...
    for (let i = 0; i < total; i++) {
      ...
      doc
        ...
        .fillColor(BRAND.gray)
        .text(
          `${byLine} · ${new Date().toLocaleString('es-CO')}`,
          PG_MARGIN,
          footerY,
          {
            width: CONTENT_W / 2,
  ...
  async buildProfitExcel(
    data: ProfitDashboardDto,
    owner: ReportOwnerDto,
  ): Promise<Buffer> {
    ...
    sheet.addRow([
      `Fondo reinversión (${data.reinvestmentPercent}%)`,
      data.totalReinvestmentFund,
    ]);
    sheet.addRow([
      `Ganancia libre dueño (${data.ownerProfitPercent}%)`,
      data.totalOwnerProfit,
    ]);
    sheet.addRow([]);
    ...
    if (data.shippingItems.length > 0) {
      sheet.addRow([]);
      sheet.addRow(['Fondo de domicilios (vehículo de entregas)']).font = {
        bold: true,
      };
      const shippingHeader = sheet.addRow([
        'Pedido',
        'Tipo',
        'Estado',
        'Zona',
      ...
      sheet.addRow(['', '', '', 'TOTAL', data.totalShippingRevenue, '']).font =
        { bold: true };
    }
  ...
  async buildProfitPdf(
    ...
    return new Promise((resolve, reject) => {
      ...
      this.drawHeader(
        ...
        `Periodo: ${periodLbl} · ${today}`,
      );

      // Summary cards — row 1
      const totalOrders = data.items.length;
      const wholesaleOrders = data.items.filter(
        (i) => i.orderType === 'MAYORISTA',
      ).length;
      const avgValue =
        totalOrders > 0 ? Math.round(data.totalRevenue / totalOrders) : 0;
      const CARD_H = 46;
      ...
      this.drawSummaryBox(
        ...
        CARD_H,
        'Ganancia Bruta',
        this.fmt(data.totalGrossProfit),
      );
      this.drawSummaryBox(
        doc,
        PG_MARGIN + (cardW2 + 4) * 1,
        cy,
        cardW2,
        CARD_H,
        'Ganancia Neta',
        this.fmt(data.totalProfit),
      );
      this.drawSummaryBox(
        doc,
        PG_MARGIN + (cardW2 + 4) * 2,
        cy,
        cardW2,
        CARD_H,
        'Comisión Secretaría',
        this.fmt(data.totalCommission),
      );
      cy += CARD_H + 6;
      ...
      this.drawSummaryBox(
        ...
        36,
        'Fondo de domicilios',
        this.fmt(data.totalShippingRevenue),
      );
      doc
        .font('Helvetica')
        .fontSize(8)
        .fillColor(BRAND.gray)
        .text(
          'Separado de ganancias de relojes · destinado a gastos del vehículo de entregas',
          PG_MARGIN + 8,
          cy + 22,
          {
            width: CONTENT_W - 16,
      ...
      if (data.items.length === 0) {
        ...
        for (let idx = 0; idx < data.items.length; idx++) {
          ...
          const cells: PdfCell[] = [
            {
              value: item.paidAt ? this.fmtDate(item.paidAt) : '—',
              x: cols[0].x,
              w: cols[0].w,
              align: 'left',
            },
            ...
            {
              value: orderTypeLabels[item.orderType] ?? item.orderType,
              x: cols[3].x,
              w: cols[3].w,
              align: 'left',
            },
            ...
            {
              value: this.fmt(item.revenue),
              x: cols[5].x,
              w: cols[5].w,
              align: 'right',
            },
            {
              value: this.fmt(item.profit),
              x: cols[6].x,
              w: cols[6].w,
              align: 'right',
            },
        ...
        const totalCells: PdfCell[] = [
          ...
          {
            value: this.fmt(data.totalRevenue),
            x: cols[5].x,
            w: cols[5].w,
            align: 'right',
          },
          {
            value: this.fmt(data.totalProfit),
            x: cols[6].x,
            w: cols[6].w,
            align: 'right',
          },
      ...
      if (data.shippingItems.length > 0) {
        ...
        doc
          ...
          .fillColor(BRAND.dark)
          .text(
            'Fondo de domicilios (vehículo de entregas)',
            PG_MARGIN,
            tableY,
        ...
        for (let idx = 0; idx < data.shippingItems.length; idx++) {
          ...
          const cells: PdfCell[] = [
            {
              value: this.fmtDate(item.paidAt),
              x: shipCols[0].x,
              w: shipCols[0].w,
              align: 'left',
            },
            ...
            {
              value: item.shippingZoneName ?? 'Sin zona',
              x: shipCols[2].x,
              w: shipCols[2].w,
              align: 'left',
            },
            {
              value: this.fmt(item.shippingCost),
              x: shipCols[3].x,
              w: shipCols[3].w,
              align: 'right',
            },
        ...
        const shipTotal: PdfCell[] = [
          ...
          {
            value: this.fmt(data.totalShippingRevenue),
            x: shipCols[3].x,
            w: shipCols[3].w,
            align: 'right',
          },
  ...
  async buildWarrantyExcel(
    data: WarrantyHistoryExportDto,
    owner: ReportOwnerDto,
  ): Promise<Buffer> {
    const workbook = new ExcelJS.Workbook();
    ...
    data.items.forEach((item) => {
      sheet.addRow([
        ...
        item.serviceDate
          ? new Date(item.serviceDate).toLocaleString('es-CO')
          : '',
  ...
  async buildWarrantyPdf(
    ...
    return new Promise((resolve, reject) => {
      ...
      this.drawHeader(
        ...
        `Periodo: ${data.periodLabel} · ${today}`,
      );

      // Summary cards
      const total = data.items.length;
      const registered = data.items.filter(
        (i) => i.status === 'GARANTIA_REGISTRADA',
      ).length;
      const CARD_H = 46;
      ...
      if (data.items.length === 0) {
        doc
          ...
          .fillColor(BRAND.gray)
          .text(
            'Sin historias de garantía en el periodo seleccionado.',
            PG_MARGIN,
            tableY + 8,
            {
              width: CONTENT_W,
        ...
        for (let idx = 0; idx < data.items.length; idx++) {
          ...
          const statusLabel =
            item.status === 'GARANTIA_REGISTRADA' ? 'Reg.' : 'Pend.';
          const cells: PdfCell[] = [
            {
              value: this.fmtDate(item.createdAt),
              x: cols[0].x,
              w: cols[0].w,
              align: 'left',
            },
            ...
            {
              value: item.serviceDate ? this.fmtDate(item.serviceDate) : '—',
              x: cols[4].x,
              w: cols[4].w,
              align: 'left',
            },
