...
export class DashboardsController {
  ...
  health(
    @Query('period')
    period?: 'day' | '2weeks' | 'week' | 'month' | '3months' | 'all',
  ) {
  ...
  @Get('revenue')
  revenue(
    @Query('range') range?: 'today' | '1_week' | '1_month' | 'historical',
  ) {
  ...
  async exportExcel(
    ...
    const buffer = await this.reportsService.buildProfitExcel(
      data,
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="LUXTIMEE-ganancia-${safePeriod}.xlsx"`,
    );
    res.send(buffer);
  }
  ...
  async exportPdf(
    ...
    const safePeriod = this.assertExportPeriod(period);
    const data = await this.dashboardsService.getProfitDashboard(safePeriod);
    const buffer = await this.reportsService.buildProfitPdf(
      data,
      this.toReportOwner(user),
    );
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="LUXTIMEE-ganancia-${safePeriod}.pdf"`,
    );
    res.send(buffer);
  }
  ...
  private assertExportPeriod(period: string): ProfitReportPeriod {
    if (!EXPORT_PERIODS.includes(period as ProfitReportPeriod)) {
      throw new BadRequestException(
        'Los reportes solo pueden exportarse por día, semana o mes.',
      );
    }
