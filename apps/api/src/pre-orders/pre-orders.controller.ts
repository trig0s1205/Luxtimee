...
export class PreOrdersController {
  ...
  @Audit('CREATE', 'PreOrder')
  createManual(@Body() dto: CreateManualPreOrderDto) {
    return this.preOrdersService.createManual(dto);
  }

  @Get('customer-hint')
  @Roles(Role.ADMIN, Role.SUPER_ADMIN)
  @UseGuards(RolesGuard)
  customerHint(@Query('phone') phone?: string) {
    if (!phone?.trim()) {
      throw new BadRequestException('Teléfono requerido');
    }
    return this.preOrdersService.findCustomerHint(phone.trim());
  }
