...
<template>
  <div>
    <AccountAccountNav />
    <UiSectionHeader label="Mi cuenta" title="Historial de pedidos" />
    <div class="space-y-4">
      <article v-for="order in orders" :key="order.id" class="border border-lux-gold/15 p-5">
        ...
        <p class="text-sm text-lux-white-dim mb-3">{{ new Date(order.createdAt).toLocaleDateString('es-CO') }}</p>
        <button type="button" class="text-xs uppercase tracking-widest text-lux-gold" @click="viewReceipt(order.id)">Ver recibo</button>
      </article>
      <p v-if="!orders?.length" class="text-lux-white-dim">Aún no tienes pedidos confirmados.</p>
    </div>
    <div v-if="selectedReceipt" class="mt-8 border border-lux-gold/30 p-6 text-sm space-y-2">
      <h4 class="font-display text-2xl text-lux-gold mb-4">Recibo digital</h4>
      <p><strong>Pedido:</strong> {{ selectedReceipt.readableId }}</p>
      <p><strong>Total:</strong> {{ formatCop(selectedReceipt.total as number) }}</p>
      <ul class="mt-2">
